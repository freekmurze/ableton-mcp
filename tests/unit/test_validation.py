...
class TestNoteValidation:
    ...
    def test_invalid_start_time_negative(self):
        ...
        # Pydantic may use custom or default error messages
        error_str = str(exc_info.value).lower()
        assert "start_time" in error_str and (
            "non-negative" in error_str or "greater than" in error_str
        )
...
class TestIndexValidation:
    ...
    def test_valid_track_index_max(self):
        ...
        from rest_api_server import MAX_TRACK_INDEX, validate_track_index
...
class TestAddNotesRequestValidation:
    ...
    def test_valid_single_note(self):
        ...
        req = AddNotesRequest(
            track_index=0, clip_index=0, notes=[Note(pitch=60, start_time=0.0, duration=0.5)]
        )
...
class TestClipLoopRequestValidation:
    ...
    def test_clip_loop_with_values(self):
        ...
        req = ClipLoopRequest(
            track_index=0, clip_index=0, loop_start=0.0, loop_end=4.0, looping=True
        )
