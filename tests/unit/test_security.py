...
class TestAPIKeyAuth:
    ...
    def test_missing_api_key_returns_401(self):
        ...
        with patch.dict(
            os.environ, {"REST_API_KEY": "test-secret-key-12345", "RATE_LIMIT_ENABLED": "false"}
        ):
            mock_conn = MagicMock()
    ...
    def test_invalid_api_key_returns_403(self):
        ...
        with patch.dict(
            os.environ, {"REST_API_KEY": "correct-api-key", "RATE_LIMIT_ENABLED": "false"}
        ):
            mock_conn = MagicMock()
            mock_conn.send_command.return_value = {}
    ...
    def test_valid_api_key_succeeds(self):
        ...
        with patch.dict(os.environ, {"REST_API_KEY": test_key, "RATE_LIMIT_ENABLED": "false"}):
            mock_conn = MagicMock()
    ...
    def test_health_endpoint_bypasses_auth(self):
        ...
        with patch.dict(
            os.environ, {"REST_API_KEY": "test-secret-key", "RATE_LIMIT_ENABLED": "false"}
        ):
            mock_conn = MagicMock()
            mock_conn.send_command.return_value = {"tempo": 120.0}
    ...
    def test_empty_api_key_disables_auth(self):
        ...
        with patch.dict(os.environ, {"REST_API_KEY": "", "RATE_LIMIT_ENABLED": "false"}):
            mock_conn = MagicMock()
            mock_conn.send_command.return_value = {"tempo": 120.0}
    ...
    def test_allowed_command_succeeds(self):
        ...
        with patch.dict(os.environ, {"RATE_LIMIT_ENABLED": "false", "REST_API_KEY": ""}):
            ...
            with patch("rest_api_server.AbletonConnection") as MockClass:
                ...
                response = client.post("/api/command", json={"command": "get_session_info"})
                assert response.status_code == 200
    ...
    def test_unknown_command_blocked(self):
        ...
        with patch.dict(os.environ, {"RATE_LIMIT_ENABLED": "false", "REST_API_KEY": ""}):
            ...
            mock_conn.send_command.side_effect = HTTPException(
                status_code=400, detail="Unknown command: malicious_command"
            )
            ...
            with patch("rest_api_server.AbletonConnection") as MockClass:
                ...
                response = client.post("/api/command", json={"command": "malicious_command"})
...
class TestCORSConfiguration:
    ...
    def test_cors_default_origins(self):
        ...
        assert "localhost" in str(rest_api_server.CORS_ORIGINS) or "127.0.0.1" in str(
            rest_api_server.CORS_ORIGINS
        )
...
class TestProxyHeaders:
    ...
    def test_proxy_headers_disabled_by_default(self):
        ...
        assert (
            rest_api_server.TRUST_PROXY_HEADERS == False
            or os.environ.get("TRUST_PROXY_HEADERS", "false").lower() == "false"
        )
    ...
    def test_x_forwarded_for_ignored_when_disabled(self):
        ...
        with patch.dict(
            ...
            with patch("rest_api_server.AbletonConnection") as MockClass:
                ...
                for _ in range(2):
                    response = client.get(
                        "/api/session", headers={"X-Forwarded-For": "192.168.1.100"}
                    )
                    assert response.status_code == 200

                # Should be rate limited
                response = client.get("/api/session", headers={"X-Forwarded-For": "192.168.1.100"})
                assert response.status_code == 429
...
class TestConnectionSecurity:
    ...
    def test_default_host_is_localhost(self):
        ...
        assert (
            rest_api_server.ABLETON_HOST == "localhost"
            or rest_api_server.ABLETON_HOST == "127.0.0.1"
        )
